...
def main():
    ...
    end_date = datetime(end_year,end_month,calendar.monthrange(end_year,end_month)[1]).date()
    ...
    while target_sheet[f'H{row_index}'].value is not None:
        ...
        usage_price_dict = make_usage_price_dict(target_sheet, row_index, usages)
        
        for usage in usages:
            for t in tx_type:
                if all(v is not None for v in usage_price_dict[usage][t].values()):
                    if usage == 'Commercial' or usage == 'Office':
                        prepare_sheet_for_com_and_office(target_wb, target_sheet, filename, row_index, t, usage, area_list, start_date, end_date, usage_price_dict, target_prop_name, result_wb_path)

                    if usage == 'Residential' or usage == 'Apartments':
                        prepare_sheet_for_residential(target_wb, target_sheet, filename, row_index, t, usage, area_list, start_date, end_date, usage_price_dict, target_prop_name, result_wb_path, residential_token)
        row_index += 1
...
def set_up_residential_token():
    ...
    try:
        ...
        token = next(filter(lambda cookie_dict: cookie_dict['name'] == 'token', cookies))['value']
        driver.close()
        return token
...
def get_sheet(wb, error=False, sname=''):
    if error:
        sname = input(f'There is no sheet called {sname}, please re-enter the name! (default: C1p.1)')
    else:
        sname = input("What's the name of the sheet you're working at? (default: C1p.1)")
    if sname == '':
...
def get_date_range(error=False):
    ...
    raw_start = input(f'Please input the start month and year of the search (format: MM/YYYY)')
    raw_end = input(f'Please input the end month and year of the search (format: MM/YYYY)')
    try:
...
def get_usages_list(target_sheet, row_index):
    ...
    for r_usage in raw_usage.split('/'):
        ...
        if r_usage not in ['Apartments','Residential','Office','Commercial']:
            print(f'Usage in row {row_index} not valid. Only Apartments, Residential, Office and Commercial are available!')
            continue
    ...
    for usage in usages:
        ...
        for t in tx_type:
            if t == 'rental':
                ...
                rental_valuation = str(target_sheet[raw_rental_valuation_cell].value) if (target_sheet[raw_rental_valuation_cell].value != 'NA') and (target_sheet[raw_rental_valuation_cell].value is not None) else None
                
                raw_rental_actual_cell = f'{usage_col_dict[usage][t]["actual"]}{row_index}'
                rental_actual = str(target_sheet[raw_rental_actual_cell].value) if (target_sheet[raw_rental_actual_cell].value != 'NA') and (target_sheet[raw_rental_actual_cell].value is not None) else None
                if (rental_valuation is None) ^ (rental_actual is None):
                    print(f'Actual or valuation price misses in row {row_index} Rental Price Valuation Summary')
                    continue
                if (rental_valuation is not None) and (rental_actual is not None):
                    valuation_range_list = [float(limit.strip().replace(',', '')) for limit in rental_valuation.split('-')]
                    actual = float(rental_actual.strip().replace(',', ''))
                    
                    valuation_range = [valuation_range_list[0]*0.9, valuation_range_list[-1]*1.1]
                ...
                selling_valuation = float(str(target_sheet[raw_selling_cell].value).strip().replace(',', '')) if (target_sheet[raw_selling_cell].value != 'NA') and (target_sheet[raw_selling_cell].value is not None) else None
...
def request_miland_commercial_data(tx, target_area, start_date, end_date, ics_type, usage_price_dict, usage, t, max_trials):
    headers = {
    ...
    final_result = None
    final_comb = None
    combs = combinations(res_json, 3) 
    
    for comb in combs:
        comb_sum = 0
        count = 0
        for c in comb:
            if t == "selling" and c.get('ft_sell') and c['ft_sell'] > 0: 
                comb_sum += c['ft_sell']
                count += 1
            elif t == 'rental' and c.get('ft_rent') and  c['ft_rent'] > 0:
                comb_sum += c['ft_rent']
                count += 1
        if count == 3:
            result = comb_sum/3
    ...
    if final_result is None or final_comb is None:
        max_trials -= 1
        if max_trials >= 0:
            new_start_date = datetime(int(start_date.split('-')[0]),int(start_date.split('-')[1]),int(start_date.split('-')[2]))+ relativedelta(months=-3)
            new_start_date = str(new_start_date.date())
            return request_miland_commercial_data(tx, target_area, new_start_date, end_date, ics_type, usage_price_dict, usage, t, max_trials)
        else:
            return [None, None]
    else:
        return [final_result, final_comb]

def prepare_sheet_for_com_and_office(target_wb, target_sheet, filename, row_index, t, usage, area_list, start_date, end_date, usage_price_dict, target_prop_name, result_wb_path):
    ...
    if len(target_area_list) > 0:
        max_trials = 5
        final_result, final_comb = request_miland_commercial_data(tx, target_area, start_date, end_date, ics_type, usage_price_dict, usage, t, max_trials)
    else: 
    ...
    if final_result is not None and final_comb is not None:
        ...
        for each in final_comb:
            ...
            result_ws[f'B{write_row}'] = f"{each.get('dist_name_zh')}, {each.get('chi_name')}, {each.get('streetno')}號, {each.get('floor_zh')}, {each.get('flat')}".strip().strip(',')
            result_ws[f'C{write_row}'] = float(f"{each.get('area')}")
            result_ws[f'D{write_row}'] = int(each.get('ft_rent')) if each.get('ft_rent') != 0 else 'NA'
            result_ws[f'E{write_row}'] = int(each.get('ft_sell')) if each.get('ft_sell') != 0 else 'NA'
        ...
        print(f'Could find row {row_index} result within 10% difference of valuation price!')

def request_miland_residential_data(tx, target_area, start_date, end_date, usage_price_dict, usage, t, max_trials, token):
    ...
    def search_date_range(place_dict):
        ...
        if not raw_tx_date:
            return False
        try:
            tx_date = datetime.strptime(raw_tx_date.split('T')[0], "%Y-%m-%d").date()
...
def prepare_sheet_for_residential(target_wb, target_sheet, filename, row_index, t, usage, area_list, start_date, end_date, usage_price_dict, target_prop_name, result_wb_path, token):
    tx = 'S' if t == 'selling' else 'L'
    ...
    if len(target_area_list) > 0:
        max_trials = 5
        final_result, final_comb = request_miland_residential_data(tx, target_area, start_date, end_date, usage_price_dict, usage, t, max_trials, token)
    else: 
        final_result, final_comb = None, None
    ...
    if final_result is not None and final_comb is not None:
        ...
        for each in final_comb:
            ...
            result_ws[f'B{write_row}'] = ','.join(address_list).strip().strip(',')
            result_ws[f'C{write_row}'] = float(f"{each.get('net_area')}")
            result_ws[f'D{write_row}'] = int(each.get('price')/each.get('net_area')) if tx == 'L' else 'NA'
            result_ws[f'E{write_row}'] = int(each.get('price')/each.get('net_area')) if tx == 'S' else 'NA'
